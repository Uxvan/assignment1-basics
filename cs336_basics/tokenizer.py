...
class Tokenizer:
    def __init__(self,vocab,merges,special_tokens: list=None):
        self.vocab=vocab
        self.merges=merges
        self.special_tokens=special_tokens or []
        ...
        self.merges_to_priority={self.merges[i]:i for i in range(len(self.merges))}
        self.piece_to_tok={} # 记录每个piece对应的list[token_id]，方便下一次遇到直接使用
        ...
        for p in parts:
            if p in self.special_tokens:
                ...
                for piece in pieces:
                    ...
                    #判断这个piece先前是否被记录过，若是，则直接在word_to_tok找对应token
                    if piece in self.piece_to_tok:
                        piece_id=self.piece_to_tok[piece]
                        all_id_to_txt.extend(piece_id)
                        continue
                    ...
                    all_id_to_txt.extend(piece_id)
                    self.piece_to_tok[piece]=piece_id
