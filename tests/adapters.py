...
def run_transformer_block(
    ...
    tfBlock=TransformerBlock(d_model,num_heads,d_ff,max_seq_len,theta)
    with torch.no_grad():
        tfBlock.mha.W_Q.weight.copy_(weights['attn.q_proj.weight'])
        tfBlock.mha.W_K.weight.copy_(weights['attn.k_proj.weight'])
        tfBlock.mha.W_V.weight.copy_(weights['attn.v_proj.weight'])
        tfBlock.mha.W_O.weight.copy_(weights['attn.output_proj.weight'])
        tfBlock.norm1.g.copy_(weights['ln1.weight'])
        tfBlock.norm2.g.copy_(weights['ln2.weight'])
        tfBlock.ffn.w1.weight.copy_(weights['ffn.w1.weight'])
        tfBlock.ffn.w2.weight.copy_(weights['ffn.w2.weight'])
        tfBlock.ffn.w3.weight.copy_(weights['ffn.w3.weight'])
...
def run_transformer_lm(
    ...
    with torch.no_grad():
        ...
        for i in range(num_layers):
            tflm.layers[i].mha.W_Q.weight.copy_(weights[f'layers.{i}.attn.q_proj.weight'])
            tflm.layers[i].mha.W_K.weight.copy_(weights[f'layers.{i}.attn.k_proj.weight'])
            tflm.layers[i].mha.W_V.weight.copy_(weights[f'layers.{i}.attn.v_proj.weight'])
            tflm.layers[i].mha.W_O.weight.copy_(weights[f'layers.{i}.attn.output_proj.weight'])
            tflm.layers[i].norm1.g.copy_(weights[f'layers.{i}.ln1.weight'])
            tflm.layers[i].norm2.g.copy_(weights[f'layers.{i}.ln2.weight'])
            tflm.layers[i].ffn.w1.weight.copy_(weights[f'layers.{i}.ffn.w1.weight'])
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    #raise NotImplementedError
    from cs336_basics.train_model import gradient_clipping
    res=gradient_clipping(parameters, max_l2_norm)
    return res
...
def run_get_lr_cosine_schedule(
    ...
    """
    #raise NotImplementedError
    from cs336_basics.train_model import cosine_lr_schedule_with_warmup
    res=cosine_lr_schedule_with_warmup(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
    return res
